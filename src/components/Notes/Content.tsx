...
import {CircularProgress, Snackbar, useTheme} from "@material-ui/core";
...
const Content = () => {
    ...
    return (
        <React.Fragment>
            <Title><FormattedMessage {...messages.notes}/></Title>
            {progress ? <SkeletonStyled variant="rect" /> :
            <Table size="small">
                <TableHead>
                    <TableRow>
                ...
                <TableBody>
                    {items && items.map((row) => (
                        <TableRow key={row.id} onClick={() => history.push(PATHS.makeNoteId(row.id))}>
